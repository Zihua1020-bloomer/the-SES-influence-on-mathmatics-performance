...
# Download and unzip the 2015 fourth grade international database-------------------
this_url <- "https://www.iea.nl/sites/default/files/data-repository/TIMSS/TIMSS2015/TIMSS2015_IDB_SPSS_G4.zip"
tf <- tempfile()
GET( this_url , write_disk( tf ) , progress() )
unzipped_files <- unzip( tf , exdir = tempdir() )

# Import and stack each of the student context data files------------------------
# limit unzipped files to those starting with `ASG` followed by three letters followed by `M6`
asg_fns <- unzipped_files[ grepl( 'ASG[A-Z][A-Z][A-Z]M6' , basename( unzipped_files ) ) ]
fns_countries <- paste0( paste0( '^ASG' , countries , 'M6' ) , collapse = "|" )
...
write.csv(all_data, "merged_data_2015.csv", row.names = FALSE)
